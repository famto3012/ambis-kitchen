// /data/page.js
import React from "react";

export const blogData = [
  {
    id: 1,
    slug: "authentic-homemade-food-trivandrum",
    title: "Ambi’s Kitchen – The Heart of Authentic Homemade Food in Trivandrum",
    excerpt:
      "Discover how Ambi’s Kitchen brings fresh, homemade, authentic Kerala food to your doorstep with love, tradition, and purity.",
    date: "Nov 30, 2025",
    readTime: "10 min read",
    category: "Featured",
    image: "/images/ambi.png",
    author: "Chef Ambi",
    isFeatured: true,
    content: `
      <p class="lead">
        In a world where fast food and rushed lifestyles dominate, finding clean, 
        comforting, <strong>homemade food in Trivandrum</strong> has become a real challenge. 
        This is exactly why <strong>Ambi’s Kitchen</strong> was created — to bring back the warmth, purity, 
        and emotional comfort of home-cooked meals.
      </p>

      <p>
        Whether you're a student missing home-cooked meals, a working professional too 
        busy to cook, or someone who simply enjoys traditional Kerala flavors, 
        Ambi’s Kitchen promises a food experience that feels like family.
      </p>
<br />

      <h2>🌿 The Story Behind Ambi’s Kitchen</h2>
      <p>
        Ambi’s Kitchen started with one simple mission — <strong>to serve food that brings people 
        back to the comfort of home</strong>. What began as a small home kitchen has now become one of 
        Trivandrum’s most trusted homemade food destinations.
      </p>

      <blockquote>
        “Good food doesn’t need luxury. It needs love.  
        At Ambi’s Kitchen, every meal is prepared with the same care you’d use for your own family.”
      </blockquote>
<br />

      <h2>🍽️ What Makes Our Homemade Food Special?</h2>
      <ul>
        <li><strong>Authentic Kerala recipes</strong> passed down through generations.</li>
        <li><strong>No preservatives or artificial flavors</strong> — only fresh ingredients.</li>
        <li><strong>Slow cooking techniques</strong> to preserve natural taste.</li>
        <li><strong>Daily fresh meals</strong> prepared in a hygienic kitchen.</li>
        <li><strong>Affordable pricing</strong> so everyone can enjoy good food daily.</li>
      </ul>
<br />

      <h2>🍛 The Ambi’s Kitchen Menu Experience</h2>
      <p>
        Our menu brings a blend of comfort food and traditional Kerala dishes. 
        Each item is crafted to give a balance of taste, health, and satisfaction.
      </p>
<br />

      <h3>⭐ 1. Kerala-Style Meals</h3>
      <p>
        Our signature meals include rice, sambar, thoran, kootu curry, avial, 
        kichadi, pappadam, and homemade pickles. A complete traditional plate 
        that reminds you of festivals and family lunches.
      </p>
<br />

      <h3>⭐ 2. Healthy Homemade Lunch Boxes</h3>
      <p>
        Perfect for office-goers who need clean, light, and nutritious food. 
        Made with minimal oil and balanced spices.
      </p>
<br />

      <h3>⭐ 3. Special Desserts & Payasams</h3>
      <p>
        From <strong>Pink Palada</strong> made through slow reduction to 
        <strong>Ada Pradhaman</strong> and <strong>Semiya Payasam</strong>, 
        our sweets capture Kerala’s festive spirit.
      </p>
<br />

      <h2>💚 Why Homemade Food Is Better Than Restaurant Food</h2>
      <p>
        Restaurant food may satisfy cravings, but homemade food nourishes the body.  
        Ambi’s Kitchen ensures:
      </p>

      <ul>
        <li>Less oil, less spice — easy on the stomach</li>
        <li>Fresh vegetables and clean ingredients</li>
        <li>Better digestion and energy levels</li>
        <li>Safe for kids, elders, and health-conscious individuals</li>
      </ul>
<br />

      <h2>🚚 Fast & Hygienic Food Delivery Across Trivandrum</h2>
      <p>
        We deliver homemade meals across Trivandrum with professional packaging, 
        careful handling, and quick delivery times. Your food reaches you warm, 
        fresh, and ready to enjoy.
      </p>
<br />

      <h2>🌟 Final Thoughts</h2>
      <p>
        Ambi’s Kitchen is not just a food service — it’s a celebration of tradition, 
        simplicity, and genuine homemade taste.  
        Every dish tells a story of love, warmth, and Kerala’s cultural richness.
      </p>

      <p><strong>
        Fresh. Honest. Homemade.  
        That’s the promise of Ambi’s Kitchen.
      </strong></p>
    `,
  },
  {
    id: 2,
    slug: "attukal-pongala-paal-payasam-seva",
    title: "Ambi’s Kitchen Serves Free Paal Payasam to Thousands of Devotees During Attukal Pongala – A Heartfelt Story of Seva, Faith & Community",
    excerpt:
      "During the world’s largest gathering of women, Attukal Pongala, Ambi’s Kitchen offered free Paal Payasam to thousands of devotees. Here is the full story behind this beautiful act of devotion and community service.",
    date: "Dec 01, 2025",
    readTime: "22 min read",
    category: "Featured",
    image: "/images/attukal.jpg",
    author: "Chef Ambi",
    isFeatured: false,
    content: `
      <p class="lead">
        Attukal Pongala is not just a festival. It is an emotion, a movement, a spiritual 
        phenomenon that transforms Thiruvananthapuram into the world’s largest gathering of 
        women. In the middle of this sacred celebration, <strong>Ambi’s Kitchen</strong> 
        stepped forward with a gesture rooted in kindness — serving <strong>FREE Paal 
        Payasam</strong> to every devotee passing by. This is the complete story of that 
        extraordinary day, the devotion behind it, and why it became one of the most 
        heartwarming events of the festival.
      </p>
<br />
      <h2>🌸 What Makes Attukal Pongala the World’s Largest Women Gathering?</h2>
      <p>
        Every year, millions of women gather around Attukal Bhagavathy Temple. Roads close, 
        houses open their compounds, and the entire city becomes a sacred cooking space. 
        Devotees come from across India — Tamil Nadu, Karnataka, Hyderabad, Mumbai, Gulf 
        returnees, and even foreigners who wish to witness something divine.
      </p>

      <p>
        There is no VIP, no class, no caste, no rich or poor. Every woman sits together on 
        the ground, lights her chulha, and prepares the Pongala offering — a symbol of 
        devotion, surrender, protection, and gratitude to Attukal Amma.
      </p>

      <p>
        It is in this powerful atmosphere of devotion that Ambi’s Kitchen decided to do 
        something meaningful for thousands who walk tirelessly in the sun.
      </p>
<br />

      <h2>🥛 The Birth of an Idea: “Let’s Serve Payasam to Devotees”</h2>
      <p>
        A week before Pongala, conversations inside Ambi’s Kitchen began revolving around 
        one idea: <strong>What can we do for the devotees?</strong> It wasn’t a business plan. 
        It wasn’t a promotional thought. It was a feeling — a desire to offer something to 
        Attukal Amma’s devotees with love.
      </p>

      <p>
        After discussions, the team agreed that nothing is purer, sweeter, or more 
        universally loved than <strong>Paal Payasam</strong>. It is simple, divine, and 
        traditionally connected to blessings.
      </p>

      <p>
        And thus, the message was decided:
        <strong>“On Pongala day, Ambi’s Kitchen will serve FREE Paal Payasam.”</strong>
      </p>
<br />

      <h2>⚡ Preparation Begins: A Massive Yet Heartfelt Effort</h2>
      <p>
        Preparing payasam for so many people is not easy. But the team was determined. 
        Fresh milk was sourced at Dawn. The highest quality raw rice was used. Sugar, 
        cardamom, and ghee were measured with care. Clean vessels were arranged in rows. 
        It wasn’t just cooking. It was a ritual performed with devotion.
      </p>

      <p>
        The kitchen was filled with the fragrance of boiling milk. Some stirred 
        continuously for hours. Others ensured hygiene and serving readiness. Everyone 
        worked with a smile because this wasn’t just food — it was seva.
      </p>
<br />

      <h2>🔥 Pongala Morning: A Divine Scene Unfolds</h2>
      <p>
        As dawn broke, devotees began pouring into the streets. Lakhs of women sat together 
        on the ground. The sound of prayers, temple bells, and chanting filled the air. 
        The roads became holy spaces glowing with clay pots and fire.
      </p>

      <p>
        And just as the sun began rising, Ambi’s Kitchen opened its serving counter with a 
        humble board:
        <strong>“Free Paal Payasam for All Devotees.”</strong>
      </p>
<br />

      <h2>🙏 The First Devotees Arrive</h2>
      <p>
        The first woman who approached was elderly. She was tired, sweating, and looking 
        for shade. When she received the first cup of payasam, her hands trembled — not 
        from weakness, but from gratitude. She took a sip, closed her eyes, and said:
      </p>
      <blockquote>
        “Mole… this is prasadam. May Amma bless you all.”
      </blockquote>

      <p>
        And just like that, a stream of devotees began arriving. Some walked miles. Some 
        carried their Pongala pots. Some had young children with them. But no one left 
        without a smile.
      </p>
<br />

      <h2>❤️ The Emotions Were More Beautiful Than the Payasam</h2>
      <p>
        Women prayed for the team. Some hugged the volunteers. Some took extra cups “for 
        their sister sitting down the lane.” Many returned just to thank the staff. It 
        created a circle of love, kindness, and blessings.
      </p>

      <p>
        Ambi’s Kitchen did not expect this response. They did not expect tears, blessings, 
        or the emotional stories devotees shared. Every moment felt sacred.
      </p>
<br />

      <h2>✨ Why This Gesture Stood Out Among Thousands of Pongala Services</h2>
      <ul>
        <li>It was 100% homemade — slow cooked like temple payasam.</li>
        <li>No branding, no advertisement — pure service.</li>
        <li>Served with devotion and respect, not for profit.</li>
        <li>Hygienic preparation ensured safety for everyone.</li>
        <li>Given exactly when devotees needed hydration and energy.</li>
        <li>Everyone was treated equally — children, elders, workers, all.</li>
      </ul>
<br />

      <h2>🌿 Voices From the Crowd: What Devotees Said</h2>
      <p>
        Hundreds shared their experiences:
      </p>
      <ul>
        <li>“Feels like temple prasadam.”</li>
        <li>“We didn’t expect this kindness.”</li>
        <li>“After cooking for hours, this was a blessing.”</li>
        <li>“Amma will bless you richly.”</li>
      </ul>
<br />

      <h2>💚 Community Members Joined the Service</h2>
      <p>
        Something magical happened — passers-by began volunteering. Young boys helped pour 
        payasam. Women distributed cups. Local residents offered water. Pedestrians 
        assisted senior devotees. The service turned into a community movement.
      </p>

      <p>
        It became more than a stall — it became a spiritual memory.
      </p>
<br />

      <h2>🔥 The Team’s Experience: “This Gave Us More Joy Than Business Ever Could”</h2>
      <p>
        At the end of the day, when the payasam finished, the team did not feel tired. 
        They felt fulfilled. There was peace in their hearts. There was satisfaction that 
        cannot be bought with money.
      </p>

      <p>
        Many staff members said:
      </p>
      <blockquote>
        “On this Pongala, we didn’t cook food. We cooked blessings.”
      </blockquote>
<br />

      <h2>🌺 A Promise for the Future</h2>
      <p>
        The overwhelming response led to a beautiful decision:
        <strong>
          From now on, every Attukal Pongala, Ambi’s Kitchen will serve free Paal Payasam 
          to devotees.
        </strong>
      </p>

      <p>
        Not as a brand. Not as a business.  
        But as a devotee offering seva to other devotees.
      </p>
<br />

      <h2>🌟 Final Thoughts: A Festival, A Kitchen & A Thousand Blessings</h2>
      <p>
        Attukal Pongala teaches us devotion.  
        But Ambi’s Kitchen reminded us of something just as important:
        <strong>kindness</strong>.
      </p>

      <p>
        A simple act — a cup of payasam — became the day’s sweetest blessing for thousands 
        of devotees. And for Ambi’s Kitchen, the blessings they received were worth far 
        more than any sale or profit.
      </p>

      <p><strong>
        Devotion is powerful.  
        But devotion shared with love becomes divine.
      </strong></p>
    `,
  },
  {
  id: 3,
  slug: "healthy-homemade-meals",
  title: "Why Homemade Food Is Better for Your Health – The Ambi’s Kitchen Way",
  excerpt:
    "A premium, in-depth guide exploring how fresh ingredients, clean cooking, and Kerala traditions make homemade food the single best choice for long-term health.",
  date: "Sep 20, 2025",
  readTime: "18 min read",
  category: "Health",
  image: "/images/hero.jpg",
  author: "Chef Ambi",
  content: `
    <p class="lead">
      In a fast-paced world ruled by convenience, takeaways, and instant meals,
      <strong>homemade food has quietly become a luxury</strong>. People are so busy
      that cooking is often replaced by packaged snacks or heavy restaurant food.
      Yet, no matter how delicious hotel meals appear, one truth always remains:
      <strong>nothing nourishes the body like freshly prepared homemade food.</strong>
      <br/><br/>
      At <strong>Ambi’s Kitchen</strong>, every meal is built on this belief.
      Our food isn’t just “tasty” — it’s <em>wholesome, balanced, and rooted in
      Kerala’s traditional healthy cooking methods</em>.
    </p>
<br />

    <h2>🌿 1. Fresh Ingredients — The Foundation of True Health</h2>
    <p>
      Health begins long before food reaches your plate.
      It begins in the market — in choosing the right vegetables, the right grains,
      the right spices. At Ambi’s Kitchen, ingredients are sourced <strong>daily</strong>.
      No frozen packets. No old stock. No chemical-ripened produce.
    </p>
    <ul>
      <li>Vegetables bought every morning from local markets</li>
      <li>Freshly grated coconut instead of packaged alternatives</li>
      <li>Homemade masalas ground in small batches</li>
      <li>No preservatives, colors, enhancers, or premixes</li>
    </ul>
    <p>
      When the ingredient is pure, the body receives pure nutrition.
      This is the heart of the Ambi’s Kitchen philosophy.
    </p>
<br />

    <h2>🍲 2. Kerala Cuisine — Naturally Balanced & Nutritious</h2>
    <p>
      Kerala food, when prepared in the traditional way, is one of the most balanced
      cuisines in India. It has the perfect combination of:
    </p>
    <ul>
      <li>Good fats from cold-pressed coconut oil</li>
      <li>Plant proteins from dal, avial, thoran, and legumes</li>
      <li>Fiber from vegetables and leafy greens</li>
      <li>Natural antioxidants from turmeric, pepper, ginger & garlic</li>
      <li>Healthy carbs from red rice and steamed rice</li>
    </ul>
    <p>
      The Ambi’s Kitchen menu remains loyal to these traditions, offering meals that
      comfort your stomach while strengthening your health.
    </p>
<br />

    <h2>💚 3. Light on Oil, Spices & Salt — Easy on the Stomach</h2>
    <p>
      Most restaurant food uses excess:
    </p>
    <ul>
      <li>oil to increase taste</li>
      <li>salt to enhance flavor</li>
      <li>spices to create strong aroma</li>
    </ul>
    <p>
      But homemade food does the opposite. It focuses on <strong>natural flavour,
      not forced richness</strong>. At Ambi’s Kitchen:
    </p>
    <ul>
      <li>Oil is used only as needed</li>
      <li>Salt is kept moderate</li>
      <li>Spices are balanced, not overpowering</li>
    </ul>
    <p>
      This makes our meals safe for:
      <strong>kids, elders, diabetic patients, working professionals, and health-conscious individuals.</strong>
    </p>
<br />

    <h2>🔥 4. No Reused Oils, No Artificial Additives</h2>
    <p>
      The biggest hidden danger in commercial cooking is reused oil.
      It is linked to:
    </p>
    <ul>
      <li>Digestive issues</li>
      <li>Acidity</li>
      <li>Cholesterol increase</li>
      <li>Inflammation</li>
    </ul>
    <p>
      At Ambi’s Kitchen, we use only <strong>fresh cold-pressed coconut oil</strong>
      and never reuse fried oil — a rare commitment in today’s cooking world.
    </p>
<br />

    <h2>👩‍🍳 5. Slow Cooking — Food That Loves Your Body</h2>
    <p>
      Traditional Kerala homes followed slow cooking. It:
    </p>
    <ul>
      <li>preserves nutrients</li>
      <li>enhances natural taste</li>
      <li>helps digestion</li>
      <li>keeps the stomach cool</li>
    </ul>
    <p>
      Ambi’s Kitchen preserves this tradition. Whether it’s
      <strong>sambar, kootu curry, chicken roast, thoran, or payasam</strong> —
      every dish is made without shortcuts.
    </p>
<br />

    <h2>🌸 6. Clean Cooking, Clean Mind — The Ambika Touch</h2>
    <p>
      The health of homemade food also depends on the <strong>energy of the kitchen</strong>.
      Ambika cooks with:
    </p>
    <ul>
      <li>Calmness</li>
      <li>Patience</li>
      <li>Clean surroundings</li>
      <li>A positive mindset</li>
    </ul>
    <p>
      Food made with love affects not just the body, but the mind.
    </p>
<br />

    <h2>🍽️ 7. Perfect for Daily Eating — Not Just “Treat Days”</h2>
    <p>
      Most restaurant foods are “once in a while” meals.
      They are too heavy to eat daily.
      But homemade meals can be eaten every day without guilt.
    </p>
    <p>
      That’s why many customers order Ambi’s Kitchen meal boxes every day.
      The food never feels oily, overwhelming, or heavy.
    </p>
<br />

    <h2>🧘 8. Supporting Weight Loss & Lifestyle Health</h2>
    <p>
      Homemade food plays a major role in:
    </p>
    <ul>
      <li>weight loss</li>
      <li>thyroid balance</li>
      <li>gut healing</li>
      <li>blood sugar control</li>
      <li>clean skin</li>
      <li>reduced bloating</li>
    </ul>
    <p>
      Ambi’s meals follow the Ayurvedic principle:
      <strong>“Food should energize, not exhaust.”</strong>
    </p>
<br />

    <h2>🍱 9. The Ambi’s Kitchen Daily Meal Experience</h2>
    <p>
      A typical Ambi’s Kitchen meal contains:
    </p>
    <ul>
      <li>Steamed rice or Kerala red rice</li>
      <li>2–3 vegetable sides (thoran, mezhukkupuratti, kichadi)</li>
      <li>One main curry (sambar, rasam, dal, or fish curry)</li>
      <li>Pickle or chutney</li>
      <li>Optional protein add-ons (chicken, fish, or egg)</li>
    </ul>
    <p>
      This combination supports digestion, improves metabolism, and keeps you full without heaviness.
    </p>
<br />

    <h2>💛 10. Why Ambi’s Kitchen Is “Doctor Recommended”</h2>
    <p>
      Many customers mention that doctors personally advised them to switch to homemade meals.
      Ambi’s Kitchen often becomes their go-to option because:
    </p>
    <ul>
      <li>the food is safe for sensitive stomachs</li>
      <li>no heavy masala is used</li>
      <li>meals maintain consistent quality</li>
    </ul>
<br />

    <h2>🏡 11. A Taste of Home for People Away from Home</h2>
    <p>
      Students, bachelors, IT employees, and working couples rely on Ambi’s Kitchen because it
      reminds them of home-cooked meals.
    </p>
    <blockquote>
      “This tastes like my mother cooked it. That’s why I order daily.”
    </blockquote>
<br />

    <h2>🌟 Final Thoughts</h2>
    <p>
      Homemade food is more than a meal — it is medicine, comfort, and culture.
      <strong>Ambi’s Kitchen</strong> brings this tradition to your doorstep with honesty,
      hygiene, and heartfelt cooking.
    </p>

    <p><strong>
      Eat homemade. Live healthy.  
      Your body will thank you — every single day.
    </strong></p>
  `,
},


 {
  id: 4,
  slug: "kerala-desserts-ambi",
  title: "The Magic of Kerala Desserts: How Ambi’s Kitchen Keeps Tradition Alive",
  excerpt:
    "From Palada to Ada Pradhaman, discover how Ambi’s Kitchen preserves Kerala’s sweet heritage with slow cooking, purity, and tradition.",
  date: "Aug 10, 2025",
  readTime: "12 min read",
  category: "Desserts",
  image: "/images/curries.jpg",
  author: "Chef Ambi",
  isFeatured: true,
  content: `
    <p>
      Kerala desserts are not just sweet dishes — they are <strong>memories of festivals</strong>,
      <strong>gifts of grandparents</strong>, and <strong>the flavour of home</strong>.
      At Ambi’s Kitchen, every payasam, pradhaman, and ada is prepared with one promise:
      <em>“If we cannot serve it to our own family, we will not serve it to our customers.”</em>
    </p>

    <br />

    <p>
      From the slow simmer of pink palada to the aroma of roasted coconut in Ada Pradhaman,
      our desserts follow the same techniques passed down through the kitchens of Kerala.
      We never use shortcuts, essence, artificial colours, or premixes.
      Only patience, purity, and traditional methods — just like Ambi has always cooked.
    </p>
  <br />
    <h2>🍮 Pink Palada – The Crown Jewel of Ambi’s Kitchen</h2>
    <p>
      Our <strong>signature Pink Palada</strong> has a loyal fan base across Trivandrum.
      The natural pink colour is not from added colour but from <strong>hours of slow cooking</strong>
      and careful caramelization.
    </p>

    <p>
      We start with <strong>freshly made ada</strong>, wash it the traditional way,
      and simmer it gently in rich milk on low flame. The process takes time,
      but the result is a creamy, glossy, nostalgic dessert that reminds you of temple feasts
      and Onam mornings. One spoon will take you back to your childhood.
    </p>
  <br />
    <h2>🥥 Ada Pradhaman – A Taste of Kerala’s Heritage</h2>
    <p>
      Ada Pradhaman is Kerala’s proudest dessert — and at Ambi’s Kitchen,
      we make it the way our mothers and grandmothers did.
    </p>

    <p>
      Each batch begins with:
      <ul>
        <li>freshly roasted coconut</li>
        <li>boiled handmade ada</li>
        <li>pure jaggery melted and filtered by hand</li>
        <li>a slow roast to bring out the deep golden flavour</li>
      </ul>
    </p>

    <p>
      No premade ada.  
      No instant jaggery syrup.  
      No synthetic flavours.  
      Only <strong>authentic Kerala taste</strong> in every bite.
    </p>
  <br />
    <h2>🍜 Semiya Payasam – The Comfort Dessert of Every Home</h2>
    <p>
      Light, gentle, and nostalgic — Semiya Payasam is the dessert every Malayali
      grew up eating. Whether it was a Sunday lunch, a birthday, or a last-minute celebration,
      this payasam always found its place on the table.
    </p>

    <p>
      At Ambi’s Kitchen, we prepare it exactly as Ambi has done for decades:
      <ul>
        <li>roasting the semiya to golden perfection</li>
        <li>boiling it slowly in milk</li>
        <li>adding just the right amount of sugar</li>
        <li>finishing with ghee-fried raisins and cashews</li>
      </ul>
      The result is balanced, aromatic, and comforting — a dessert that suits all ages.
    </p>
  <br />
    <h2>🥄 Why Ambi’s Kitchen Desserts Taste Different</h2>
    <p>
      Customers often ask us why our payasam tastes so homely and soothing.
      The answer is simple:
    </p>

    <ul>
      <li><strong>Fresh ingredients only — no stored or leftover items</strong></li>
      <li><strong>No shortcuts or commercial cooking methods</strong></li>
      <li><strong>Traditional slow-cooking techniques</strong></li>
      <li><strong>Perfect balance of sweetness</strong></li>
      <li><strong>Prepared in small batches for quality</strong></li>
      <li><strong>Every step personally supervised by Ambi</strong></li>
    </ul>

    <p>
      To us, desserts are not a “menu item.”  
      They are a <strong>celebration of Kerala’s culture and purity</strong>.
    </p>
  <br />
    <h2>🎉 Festivals & Special Occasions</h2>
    <p>
      During Onam, Vishu, birthdays, and family gatherings, Ambi’s Kitchen receives
      hundreds of payasam orders — because our customers know that the love,
      consistency, and authenticity remain the same every time.
    </p>

    <p>
      Whether it’s for a celebration, a temple offering, a birthday,
      or simply a craving for something sweet, Ambi’s Kitchen ensures that
      a bowl of traditional Kerala dessert reaches your home with warmth.
    </p>

    <br />

    <p>
      <strong>
        At Ambi’s Kitchen, dessert is more than food.  
        It is culture. It is love. It is Kerala.
      </strong>
    </p>
  `,
}

  // id 3 and id 4 content would follow exactly like in your original post
];

export default function DataPage() {
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-4xl font-bold mb-8">Blog Posts</h1>
      {blogData.map((blog) => (
        <div key={blog.id} className="mb-12 border-b pb-6">
          <h2 className="text-2xl font-semibold mb-2">{blog.title}</h2>
          <p className="text-gray-600 mb-1">{blog.excerpt}</p>
          <p className="text-sm text-gray-400 mb-4">
            {blog.date} • {blog.readTime} • {blog.category} • By {blog.author}
          </p>
          {blog.image && (
            <img
              src={blog.image}
              alt={blog.title}
              className="w-full max-h-80 object-cover mb-4 rounded"
            />
          )}
          <div
            className="prose max-w-full"
            dangerouslySetInnerHTML={{ __html: blog.content }}
          />
        </div>
      ))}
    </div>
  );
}



// export const blogData = [
//   {
//     id: 1,
//     slug: "authentic-homemade-food-trivandrum",
//     title: "Ambi’s Kitchen – The Heart of Authentic Homemade Food in Trivandrum",
//     excerpt:
//       "Discover how Ambi’s Kitchen brings fresh, homemade, authentic Kerala food to your doorstep with love, tradition, and purity.",
//     date: "Nov 30, 2025",
//     readTime: "10 min read",
//     category: "Featured",
//     image: "/images/ambi.png",
//     author: "Chef Ambi",
//     isFeatured: true,
//    content: `
//       <p class="lead">
//         In a world where fast food and rushed lifestyles dominate, finding clean, 
//         comforting, <strong>homemade food in Trivandrum</strong> has become a real challenge. 
//         This is exactly why <strong>Ambi’s Kitchen</strong> was created — to bring back the warmth, purity, 
//         and emotional comfort of home-cooked meals.
//       </p>

//       <p>
//         Whether you're a student missing home-cooked meals, a working professional too 
//         busy to cook, or someone who simply enjoys traditional Kerala flavors, 
//         Ambi’s Kitchen promises a food experience that feels like family.
//       </p>
// <br />

//       <h2>🌿 The Story Behind Ambi’s Kitchen</h2>
//       <p>
//         Ambi’s Kitchen started with one simple mission — <strong>to serve food that brings people 
//         back to the comfort of home</strong>. What began as a small home kitchen has now become one of 
//         Trivandrum’s most trusted homemade food destinations.
//       </p>

//       <blockquote>
//         “Good food doesn’t need luxury. It needs love.  
//         At Ambi’s Kitchen, every meal is prepared with the same care you’d use for your own family.”
//       </blockquote>
// <br />

//       <h2>🍽️ What Makes Our Homemade Food Special?</h2>
//       <ul>
//         <li><strong>Authentic Kerala recipes</strong> passed down through generations.</li>
//         <li><strong>No preservatives or artificial flavors</strong> — only fresh ingredients.</li>
//         <li><strong>Slow cooking techniques</strong> to preserve natural taste.</li>
//         <li><strong>Daily fresh meals</strong> prepared in a hygienic kitchen.</li>
//         <li><strong>Affordable pricing</strong> so everyone can enjoy good food daily.</li>
//       </ul>
// <br />

//       <h2>🍛 The Ambi’s Kitchen Menu Experience</h2>
//       <p>
//         Our menu brings a blend of comfort food and traditional Kerala dishes. 
//         Each item is crafted to give a balance of taste, health, and satisfaction.
//       </p>
// <br />

//       <h3>⭐ 1. Kerala-Style Meals</h3>
//       <p>
//         Our signature meals include rice, sambar, thoran, kootu curry, avial, 
//         kichadi, pappadam, and homemade pickles. A complete traditional plate 
//         that reminds you of festivals and family lunches.
//       </p>
// <br />

//       <h3>⭐ 2. Healthy Homemade Lunch Boxes</h3>
//       <p>
//         Perfect for office-goers who need clean, light, and nutritious food. 
//         Made with minimal oil and balanced spices.
//       </p>
// <br />

//       <h3>⭐ 3. Special Desserts & Payasams</h3>
//       <p>
//         From <strong>Pink Palada</strong> made through slow reduction to 
//         <strong>Ada Pradhaman</strong> and <strong>Semiya Payasam</strong>, 
//         our sweets capture Kerala’s festive spirit.
//       </p>
// <br />

//       <h2>💚 Why Homemade Food Is Better Than Restaurant Food</h2>
//       <p>
//         Restaurant food may satisfy cravings, but homemade food nourishes the body.  
//         Ambi’s Kitchen ensures:
//       </p>

//       <ul>
//         <li>Less oil, less spice — easy on the stomach</li>
//         <li>Fresh vegetables and clean ingredients</li>
//         <li>Better digestion and energy levels</li>
//         <li>Safe for kids, elders, and health-conscious individuals</li>
//       </ul>
// <br />

//       <h2>🚚 Fast & Hygienic Food Delivery Across Trivandrum</h2>
//       <p>
//         We deliver homemade meals across Trivandrum with professional packaging, 
//         careful handling, and quick delivery times. Your food reaches you warm, 
//         fresh, and ready to enjoy.
//       </p>
// <br />

//       <h2>🌟 Final Thoughts</h2>
//       <p>
//         Ambi’s Kitchen is not just a food service — it’s a celebration of tradition, 
//         simplicity, and genuine homemade taste.  
//         Every dish tells a story of love, warmth, and Kerala’s cultural richness.
//       </p>

//       <p><strong>
//         Fresh. Honest. Homemade.  
//         That’s the promise of Ambi’s Kitchen.
//       </strong></p>
//     `,
//   },

// {
//   id: 2,
//   slug: "attukal-pongala-paal-payasam-seva",
//   title: "Ambi’s Kitchen Serves Free Paal Payasam to Thousands of Devotees During Attukal Pongala – A Heartfelt Story of Seva, Faith & Community",
//   excerpt:
//     "During the world’s largest gathering of women, Attukal Pongala, Ambi’s Kitchen offered free Paal Payasam to thousands of devotees. Here is the full story behind this beautiful act of devotion and community service.",
//   date: "Dec 01, 2025",
//   readTime: "22 min read",
//   category: "Featured",
//   image: "/images/attukal.jpeg",
//   author: "Chef Ambi",
//   isFeatured: false,

//   content: `
//     <p class="lead">
//       Attukal Pongala is not just a festival. It is an emotion, a movement, a spiritual 
//       phenomenon that transforms Thiruvananthapuram into the world’s largest gathering of 
//       women. In the middle of this sacred celebration, <strong>Ambi’s Kitchen</strong> 
//       stepped forward with a gesture rooted in kindness — serving <strong>FREE Paal 
//       Payasam</strong> to every devotee passing by. This is the complete story of that 
//       extraordinary day, the devotion behind it, and why it became one of the most 
//       heartwarming events of the festival.
//     </p>
// <br />
//     <h2>🌸 What Makes Attukal Pongala the World’s Largest Women Gathering?</h2>
//     <p>
//       Every year, millions of women gather around Attukal Bhagavathy Temple. Roads close, 
//       houses open their compounds, and the entire city becomes a sacred cooking space. 
//       Devotees come from across India — Tamil Nadu, Karnataka, Hyderabad, Mumbai, Gulf 
//       returnees, and even foreigners who wish to witness something divine.
//     </p>

//     <p>
//       There is no VIP, no class, no caste, no rich or poor. Every woman sits together on 
//       the ground, lights her chulha, and prepares the Pongala offering — a symbol of 
//       devotion, surrender, protection, and gratitude to Attukal Amma.
//     </p>

//     <p>
//       It is in this powerful atmosphere of devotion that Ambi’s Kitchen decided to do 
//       something meaningful for thousands who walk tirelessly in the sun.
//     </p>
// <br />

//     <h2>🥛 The Birth of an Idea: “Let’s Serve Payasam to Devotees”</h2>
//     <p>
//       A week before Pongala, conversations inside Ambi’s Kitchen began revolving around 
//       one idea: <strong>What can we do for the devotees?</strong> It wasn’t a business plan. 
//       It wasn’t a promotional thought. It was a feeling — a desire to offer something to 
//       Attukal Amma’s devotees with love.
//     </p>

//     <p>
//       After discussions, the team agreed that nothing is purer, sweeter, or more 
//       universally loved than <strong>Paal Payasam</strong>. It is simple, divine, and 
//       traditionally connected to blessings.
//     </p>

//     <p>
//       And thus, the message was decided:
//       <strong>“On Pongala day, Ambi’s Kitchen will serve FREE Paal Payasam.”</strong>
//     </p>
// <br />

//     <h2>⚡ Preparation Begins: A Massive Yet Heartfelt Effort</h2>
//     <p>
//       Preparing payasam for so many people is not easy. But the team was determined. 
//       Fresh milk was sourced at Dawn. The highest quality raw rice was used. Sugar, 
//       cardamom, and ghee were measured with care. Clean vessels were arranged in rows. 
//       It wasn’t just cooking. It was a ritual performed with devotion.
//     </p>

//     <p>
//       The kitchen was filled with the fragrance of boiling milk. Some stirred 
//       continuously for hours. Others ensured hygiene and serving readiness. Everyone 
//       worked with a smile because this wasn’t just food — it was seva.
//     </p>
// <br />

//     <h2>🔥 Pongala Morning: A Divine Scene Unfolds</h2>
//     <p>
//       As dawn broke, devotees began pouring into the streets. Lakhs of women sat together 
//       on the ground. The sound of prayers, temple bells, and chanting filled the air. 
//       The roads became holy spaces glowing with clay pots and fire.
//     </p>

//     <p>
//       And just as the sun began rising, Ambi’s Kitchen opened its serving counter with a 
//       humble board:
//       <strong>“Free Paal Payasam for All Devotees.”</strong>
//     </p>
// <br />

//     <h2>🙏 The First Devotees Arrive</h2>
//     <p>
//       The first woman who approached was elderly. She was tired, sweating, and looking 
//       for shade. When she received the first cup of payasam, her hands trembled — not 
//       from weakness, but from gratitude. She took a sip, closed her eyes, and said:
//     </p>
//     <blockquote>
//       “Mole… this is prasadam. May Amma bless you all.”
//     </blockquote>

//     <p>
//       And just like that, a stream of devotees began arriving. Some walked miles. Some 
//       carried their Pongala pots. Some had young children with them. But no one left 
//       without a smile.
//     </p>
// <br />

//     <h2>❤️ The Emotions Were More Beautiful Than the Payasam</h2>
//     <p>
//       Women prayed for the team. Some hugged the volunteers. Some took extra cups “for 
//       their sister sitting down the lane.” Many returned just to thank the staff. It 
//       created a circle of love, kindness, and blessings.
//     </p>

//     <p>
//       Ambi’s Kitchen did not expect this response. They did not expect tears, blessings, 
//       or the emotional stories devotees shared. Every moment felt sacred.
//     </p>
// <br />

//     <h2>✨ Why This Gesture Stood Out Among Thousands of Pongala Services</h2>
//     <ul>
//       <li>It was 100% homemade — slow cooked like temple payasam.</li>
//       <li>No branding, no advertisement — pure service.</li>
//       <li>Served with devotion and respect, not for profit.</li>
//       <li>Hygienic preparation ensured safety for everyone.</li>
//       <li>Given exactly when devotees needed hydration and energy.</li>
//       <li>Everyone was treated equally — children, elders, workers, all.</li>
//     </ul>
// <br />

//     <h2>🌿 Voices From the Crowd: What Devotees Said</h2>
//     <p>
//       Hundreds shared their experiences:
//     </p>
//     <ul>
//       <li>“Feels like temple prasadam.”</li>
//       <li>“We didn’t expect this kindness.”</li>
//       <li>“After cooking for hours, this was a blessing.”</li>
//       <li>“Amma will bless you richly.”</li>
//     </ul>
// <br />

//     <h2>💚 Community Members Joined the Service</h2>
//     <p>
//       Something magical happened — passers-by began volunteering. Young boys helped pour 
//       payasam. Women distributed cups. Local residents offered water. Pedestrians 
//       assisted senior devotees. The service turned into a community movement.
//     </p>

//     <p>
//       It became more than a stall — it became a spiritual memory.
//     </p>
// <br />

//     <h2>🔥 The Team’s Experience: “This Gave Us More Joy Than Business Ever Could”</h2>
//     <p>
//       At the end of the day, when the payasam finished, the team did not feel tired. 
//       They felt fulfilled. There was peace in their hearts. There was satisfaction that 
//       cannot be bought with money.
//     </p>

//     <p>
//       Many staff members said:
//     </p>
//     <blockquote>
//       “On this Pongala, we didn’t cook food. We cooked blessings.”
//     </blockquote>
// <br />

//     <h2>🌺 A Promise for the Future</h2>
//     <p>
//       The overwhelming response led to a beautiful decision:
//       <strong>
//         From now on, every Attukal Pongala, Ambi’s Kitchen will serve free Paal Payasam 
//         to devotees.
//       </strong>
//     </p>

//     <p>
//       Not as a brand. Not as a business.  
//       But as a devotee offering seva to other devotees.
//     </p>
// <br />

//     <h2>🌟 Final Thoughts: A Festival, A Kitchen & A Thousand Blessings</h2>
//     <p>
//       Attukal Pongala teaches us devotion.  
//       But Ambi’s Kitchen reminded us of something just as important:
//       <strong>kindness</strong>.
//     </p>

//     <p>
//       A simple act — a cup of payasam — became the day’s sweetest blessing for thousands 
//       of devotees. And for Ambi’s Kitchen, the blessings they received were worth far 
//       more than any sale or profit.
//     </p>

//     <p><strong>
//       Devotion is powerful.  
//       But devotion shared with love becomes divine.
//     </strong></p>
//   `,
// },



// ];
